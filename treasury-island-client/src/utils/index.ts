...
export const bigintToEntity = (v: BigNumberish): Entity =>
  getEntityIdFromKeys([BigInt(v)]) as Entity;
export const keysToEntity = (keys: BigNumberish[]): Entity =>
  getEntityIdFromKeys(keys.map((v) => BigInt(v ?? 0))) as Entity;
export const entityIdToKey = (
  component: Component,
  keyName: string,
  entityId: Entity
) => BigInt(getComponentValue(component, entityId)[keyName]);
